...
final readonly class ConversionController
{
    ...
    public function __invoke(Request $request): Response
    {
        $content = is_string($request->post['lyrics'] ?? null)
            ? $request->post['lyrics']
            : '';
        $repair = isset($request->post['repair']);
        $sourceName = 'subtitles.txt';

        try {
            if (
                isset($request->files['lyrics_file'])
                && is_array($request->files['lyrics_file'])
                && (($request->files['lyrics_file']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE)
            ) {
        ...
        $_SESSION['_downloads'][$token] = [
            'content' => $srt,
            'filename' => $this->filenames->srt($sourceName),
        ];
