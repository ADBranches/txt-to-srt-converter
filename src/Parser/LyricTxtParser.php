...
final readonly class LyricTxtParser
{
    ...
    public function parse(string $content, bool $repair = false): array
    {
        ...
        foreach (explode("\n", $content) as $offset => $rawLine) {
            ...
            if ($parts === false || count($parts) !== 3) throw ConversionException::forLine($number, 'expected START | END | TEXT.');
            [$start, $end, $text] = array_map('trim', $parts);
            if ($text === '') throw ConversionException::forLine($number, 'caption text cannot be empty.');
        ...
        if ($result === []) throw new ConversionException('Input contains no subtitle entries.');
